...
2. Add overlap between chunks (planned default: `5s`) to improve stitching quality
...
Example chunk windows for a `60s` chunk with `5s` overlap:
- `0s - 60s`
- `55s - 120s`
- `115s - 180s`
...
- The extension prototype now drives chunk capture/upload; backend remains focused on per-chunk timed text generation.
